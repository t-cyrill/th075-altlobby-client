...
public class JNaturalTextPane extends JPanel {
	...
	/**
	 * <h1>setEditable</h1>
	 * <p>内部の TextComponent が編集可能かどうかを設定します。</p>
	 * <p>状態が変更されると、PropertyChange イベント (editable) が発生します。</p>
	 *
	 * @param b 設定される boolean
	 */
	...
	/**
	 * <p>指定されたテキストをドキュメントの末尾に追加します。</p>
	 * <p>このメソッドは例外的にスレッドセーフです。{@link JTextArea#append(String)}を参照してください。</p>
	 *
	...
	class JDefaultMenuForEditControl extends JPopupMenu {
		...
		private JMenuItem undoMenuItem,cutMenuItem,copySelectedMenuItem,SelectALLMenuItem
			,pasteMenuItem;
		public JDefaultMenuForEditControl() {
			...
			copySelectedMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.CTRL_DOWN_MASK));
	...
	/**
	 * キャレットの位置を行末に移動します。
	 * この操作を行うことで、テキストコンポーネントのスクロールが行われ、行末が表示されます。
	 *
	 */
