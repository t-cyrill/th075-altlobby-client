...
public class CasterSettingPanel extends JPanel {
	...
	public CasterSettingPanel(CasterSetting setting) {
		...
		JLabel casterPathLabel = new JLabel(resource.getString("config.tab.caster.path"));
		...
		JLabel casterPortLabel = new JLabel(resource.getString("config.tab.caster.port"));
	...
	public void registerLisners() {
		...
		casterChooseButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				filechooser.setDialogTitle(resource.getString("config.tab.caster.chooser_caption"));
	 ...
	 *
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはSwingテキストコンポーネントのgetTextを呼び出します。
	 * このメソッドはSwingのイベントディスパッチスレッドから呼び出さなければなりません。</p>
	 *
	 * @return このパネルに乗っているコンポーネントの設定値
	 */
	public CasterSetting getSettingValues() {
		return new CasterSetting(casterPathTextField.getText(), casterPortTextField.getText());
	}
