...
public class MessageInputPanel extends JPanel {
    ...
    public void registerListeners(Runnable enterPushedInInnerTextAreaRunnable,
        ...
        bottomPanel.registerListeners(gatherButtonPushedRunnable,
    ...
    /**
     * 内部のボタン状態を切り替える <h3>スレッドセーフ</h3>
     * <p>
     * このメソッドは {@link SwingUtilities#invokeLater(Runnable)} を呼び出します。
     * Swingのイベントディスパッチスレッド以外からでも呼び出すことができます。
     * </p>
     * 
     * @param i
     *            0 : 募集ボタンのみ有効 1 : 閉じるボタンのみ有効 2 : 両方のボタンを無効にする
     */
    ...
    /**
     * <h1>setText</h1> <h2>JTextComponent#setTextを呼び出す</h2>
     * <p>
    ...
    /**
     * <h1>setUserStateTextLabelState</h1> <h2>ラベルテキストを変更する</h2>
     * <p>
     * ユーザー状態を表すラベルのテキストを変更します。
     * </p>
     * <p>
     * このメソッドで設定されるテキストは{@link UserInfo#getStateString(int)}に依存します。
     * </p>
     * <h3>スレッドセーフ</h3>
     * <p>
     * このメソッドはSwingのイベントディスパッチスレッド以外からでも呼び出すことができます。
     * </p>
     * 
     * @param state
     *            ユーザーの状態
    ...
    class innerTopPanel extends JPanel {
        ...
        public innerTopPanel(ClientSetting setting) {
            ...
            JLabel toIMLabel = new JLabel(
                    resource.getString("main.MessageInput.toIM"));
            wholeIMCheck = new JCheckBox(
                    resource.getString("main.MessageInput.wholeIM"));
    ...
    static class innerCenterPanel extends JPanel {
        ...
        public void registerListeners(
                final Runnable enterPushedInInnerTextAreaRunnable) {
                    ...
                    if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                        ...
                        if ((sendIMKey == 0) && (mod == 0)) {
                            ...
                            send_flag = true;
                                ...
                                && ((mod & InputEvent.SHIFT_DOWN_MASK) == InputEvent.SHIFT_DOWN_MASK)) {
                            // Shift + Enter
                            send_flag = true;
                        }
    ...
    class innerBottomPanel extends JPanel {
        ...
        public innerBottomPanel() {
            ...
            gatherButton = new JButton(
                    resource.getString("main.MessageInput.button.gather"));
            gatherSwitchButton = new JButton("..");
            stateLabel = new JLabel(UserInfo.getStateString(UserInfo.WAITING),
                    JLabel.CENTER);
            closeCasterButton = new JButton(
                    resource.getString("main.MessageInput.button.close"));
        ...
        public void registerListeners(Runnable gatherButtonPushedRunnable,
                Runnable gatherSwitchButtonPushedRunnable,
                Runnable closeCasterButtonPushedRunnable) {
            gatherButton.addActionListener(new RunnableCallActionListener(
                    gatherButtonPushedRunnable));
            gatherSwitchButton
                    .addActionListener(new RunnableCallActionListener(
                            gatherSwitchButtonPushedRunnable));
            closeCasterButton.addActionListener(new RunnableCallActionListener(
                    closeCasterButtonPushedRunnable));
        }
        ...
        public void setGatherButtonText(boolean hostOnlyFlag) {
            ...
            if (hostOnlyFlag) {
                text = resource
                        .getString("main.MessageInput.button.gatherHostOnly");
            } else {
