...
public class UserInfoTableModel extends DefaultTableModel {
	...
	public static final String[] columnHeadersString = {
		"名前",
		"状態",
		"IP",
		"uid",
	...
	public void add(UserInfo p) {
		...
		String[] newUserInfo = {
				p.getPublicName(),
				UserInfo.getStateString(p.getState()),
				IPString, p.getUID(),
	...
	/**
	 * row行目のUserInfoを取得します.
	 * このメソッドはrowが範囲外の場合にnullを返します。
	 *
	 * @param row 取得する行番号
	...
	/**
	 * <h1>remove</h1>
	 * <h2>モデルに含まれるUserInfoを取り除く</h2>
	 * <p>pに一致するユーザー情報をこのモデルから取り除きます</p>
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはテーブルモデルを書き換えます。Swingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
	 *
	 * @param p 取り除く要素
	...
	/**
	 * <h1>removeAll</h1>
	 * <h2>モデルに含まれるすべてのUserInfoを取り除く</h2>
	 * <p>このモデルに含まれるすべてのUserInfoを取り除きます</p>
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはテーブルモデルを書き換えます。Swingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
	 */
	...
	/**
	 * <h1>update</h1>
	 * <h2>UIDが一致するユーザーの状態を書き換える</h2>
	 * <p>ユーザーの状態を書き換えます</p>
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはテーブルモデルを書き換えます。Swingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
	 *
	 * @param UID 書き換えるユーザーのUID
	 * @param state 新しい状態
	 */
	 ...
	 * <p>このテーブルモデルを新たに設定します。このメソッド呼び出し前のすべての操作はこのメソッドにより破棄されます。</p>
	 * <p>このメソッドはUserInfoの配列をアトミックに設定します。並行処理される複数のスレッドから安全に呼び出すことができますが、
	 * Swingの制約からEDT以外から呼び出すことは推奨されません。</p>
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはテーブルモデルを書き換えますが、内部でSwingUtilities.invokeAndWaitを呼び出すため、Swingのイベントディスパッチスレッドから呼び出すことはできません。</p>
	 *
	...
	/**
	 * <h1>ColumnContext</h1>
	 * <p>http://terai.xrea.jp/Swing/LineFocusTable.htmlを参考にしました。</p>
	 */
    private static class ColumnContext {
    	public final String  columnName;
    	public final Class<String> columnClass;
    	public final boolean isEditable;
    	public ColumnContext(String columnName, Class<String> columnClass, boolean isEditable) {
	 ...
	 * <p>内部Listを走査しUIDからUserInfoを取得します。</p>
	 * <p>一致するUIDが存在しない場合やUIDがnullの場合、このメソッドはnullを返します。</p>
	 *
     * @param UID 検索対象のUID
     * @return ユーザー情報
     */
