...
public class SettingWindow extends JFrame {
    ...
    private final ArrayBlockingQueue<Boolean> queue = new ArrayBlockingQueue<Boolean>(
            5);
    ...
    public SettingWindow(ClientSetting setting) {
        ...
        // タブペイン
        connectionSetting = new ConnectionSettingPanel(
        ...
        IMSoundSetting = new InstantMessageSoundSettingPanel(
                setting.getImsoundSetting());

        casterSetting.registerLisners();
        IMSoundSetting.registerListeners();

        JTabbedPane tabbedPane = new JTabbedPane();
        tabbedPane.addTab(resource.getString("config.tab.connection"),
                connectionSetting);
        tabbedPane.addTab(resource.getString("config.tab.caster"),
                casterSetting);
        tabbedPane.addTab(resource.getString("config.tab.IM"), IMSetting);
        tabbedPane.addTab(resource.getString("config.tab.IMsound"),
                IMSoundSetting);
