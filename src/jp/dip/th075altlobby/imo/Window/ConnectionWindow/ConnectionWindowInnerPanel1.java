...
/**
 * <h1>ConnectionWindowInnerPanel1</h1>
 * <h2>ConnectionWindowで使われるパネル</h2>
 * <p>ConnectionWindowで使われるパネルです。</p>
 * <p>接続先のJLabel、接続先を選択するJComboBox、サーバーへ接続するJButton、設定ウィンドウを開くJButtonを持ちます。</p>
 *
 * @author Cyrill
 */
public class ConnectionWindowInnerPanel1 extends JPanel {
	...
	/**
	 * <h1>ConnectionWindowInnerPanel1</h1>
	 * <h2>このクラスのインスタンスを生成する</h2>
	 * <p>インスタンスを生成します。パネルにコンポーネントを配置します。</p>
	 * <p>イベントリスナの登録はされません。インスタンスのregisterLisnerを呼び出します。</p>
	 *
	 * @param server_list コンボボックスに設定する文字列
	 */
	public ConnectionWindowInnerPanel1(String[] server_list) {
		...
		JLabel label = new JLabel(resource.getString("connection.label.connect"));
		...
		connectionButton = new JButton(resource.getString("connection.button.connect"));
		settingButton = new JButton(resource.getString("connection.button.config"));
	...
	/**
	 * <h1>WheelListener</h1>
	 * <h2>内部のコンボボックスで利用されるマウスのホイールイベントリスナ</h2>
	 * <p>内部のコンボボックスで利用されるマウスのホイールイベントリスナです。</p>
	 * <p>登録されたコンポーネントの上でマウスのホイールがされたときにそのホイール分コンボボックスをスライドホイールさせます。</p>
	 *
	 * @author Cyrill
	 */
	...
	/**
	 * <h1>registerLisners</h1>
	 * <h2>内部のSwingコンポーネントにリスナーを登録する</h2>
	 * <p>内部のSwingコンポーネントにリスナーを登録します。コンストラクタから呼び出すことはできません。</p>
	 * <p>指定されるRunnableは別スレッドで実行されます。Swingのイベントディスパッチスレッドからは呼ばれません。</p>
	 *
	 */
	public void registerLisners(Runnable connectionButtonRunnable, Runnable settingButtonRunnable) {
		connectionButton.addActionListener(new RunnableCallActionListener(connectionButtonRunnable));
		settingButton.addActionListener(new RunnableCallActionListener(settingButtonRunnable));
		server_list_box.addMouseWheelListener(new WheelListener());
	}

	/**
	 * <h1>getSelectedString</h1>
	 * <h2>コンボボックスで選択された文字列を取得する</h2>
	 * <p>内部のコンボボックスで選択された文字列を取得します。</p>
	 * <p>このメソッドはJComboBox.getSelectedIndexを呼び出すためSwingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
	 *
	 * @return 選択された文字列
	 */
	...
	/**
	 * <h1>setServerList</h1>
	 * <h2>コンボボックスに表示される文字列を設定する</h2>
	 * <p>内部のJComboBoxで使用される文字列を設定します。</p>
	 *
	 * @param list コンボボックスに表示される文字列
	 */
