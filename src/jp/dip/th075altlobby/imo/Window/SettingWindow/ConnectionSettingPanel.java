...
public class ConnectionSettingPanel extends JPanel {
    ...
    public ConnectionSettingPanel(ConnectionSetting setting) {
        ...
        JLabel nameLabel = new JLabel(
                resource.getString("config.tab.connection.name"));
        JLabel shortMessageLabel = new JLabel(
                resource.getString("config.tab.connection.short_msg"));
        JLabel messageLabel = new JLabel(
                resource.getString("config.tab.connection.message"));

        nameLabel.setBounds(10, 10, 100, 20);
        shortMessageLabel.setBounds(10, 60, 100, 20);
        messageLabel.setBounds(10, 105, 100, 20);

        IPHideCheckBox = new JCheckBox(
                resource.getString("config.tab.connection.IPhide"));
    ...
    public ConnectionSetting getSettingValues() {
        return new ConnectionSetting(nameTextField.getText(),
                messageTextField.getText(), shortMessageTextField.getText(),
                IPHideCheckBox.isSelected());
    }
}
