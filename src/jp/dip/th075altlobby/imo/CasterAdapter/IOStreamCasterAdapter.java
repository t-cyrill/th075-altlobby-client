 ...
 * <p>ProcessのIOストリームを使いCasterとの情報交換を行う機能を提供します。</p>
 * <p>このクラスはCasterの出力を拾いイベントにラップします。
 * 必要なタイミングでイベントは{@link CasterEventInterface}または{@link StateChangeListener}に通知されます。</p>
 *
 * @version rev27
 * @author Cyrill
 *
...
public class IOStreamCasterAdapter extends AbstractCasterProcessAdapter {
	...
	public IOStreamCasterAdapter(CasterEventInterface eventInterface, StateChangeListener listener, String... command) throws IOException {
		_process = new ProcessAdapter(new CasterOutputRunnable(), new ClosedRunnable(), command);
	...
	/**
	 * <h2>プロセスを終了する</h2>
	 * <p>管理しているcasterプロセスを終了させます。(<strong>任意のオペレーション</strong>)</p>
	 * <p>このメソッドのオペレーションをサポートするクラスはプロセスの終了が確認できるまでブロックします。</p>
	 *
	 */
	public void close() {
		_process.close();
	 ...
	 * <p>Casterの入力ストリームに書き出します。文字列sの後ろには改行コード\nが自動的に付加されます。</p>
	 * <p>このメソッドは柔軟な操作を行う機能を提供する一方で、想定されない操作を実行する可能性があります。
	 * 一般にはカプセル化された{@link IOStreamCasterAdapter#fireInputedMarginEvent(int)}のようなイベント着火型のメソッドを呼び出してください。
	 * イベントカプセル化を行うクラスはこのメソッドを呼び出すべきではありません。</p>
	 * <p>このメソッドは主にユーザーの手動でのオペレートを可能にするために提供されます。</p>
	 *
	 * @param s 書き出す文字列
	...
	protected class CasterOutputRunnable extends CallBackRunnable {
		...
		public void run(String s) {
			...
			// 全タイプ待ち側
			else if(s.matches("Buffer margin : (\\d+) \\( Observed about (.*)\\[.*"))
...
//				eventInterface.portSetting();
		...
		private void bufferMarginPrinted(String s) {
			Pattern pt = Pattern.compile("debug : Buffer Margin ( delayTime ) (\\d+)");
			Matcher matcher = pt.matcher(s);
			if (matcher.matches())
				_eventInterface.bufferMarginPrinted(Integer.parseInt(matcher.group(1)));
		}
		...
		private void inputedBufferMarginWithObservedDelay(String s) {
			Pattern pt = Pattern.compile("Buffer margin : (\\d+) \\( Observed about (.*)\\[.*");
			Matcher matcher = pt.matcher(s);
			if(matcher.matches()){
				int delay_buffer = Integer.parseInt(matcher.group(1));
		...
		private void existsDelay(String s) {
			Pattern pt = Pattern.compile("About (.*)\\[ms\\] delay exist in a round\\.");
			Matcher matcher = pt.matcher(s);
			double real_delay = -1;
			if(matcher.matches())
