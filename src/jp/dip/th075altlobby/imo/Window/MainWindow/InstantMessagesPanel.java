...
public class InstantMessagesPanel extends JPanel {
	...
	private final JMenuItem copySelectedMenuItem = new JMenuItem(resource.getString("main.InstantMessage.menu.copy"));
	private final JMenuItem selectALLMenuItem = new JMenuItem(resource.getString("main.InstantMessage.menu.AllSel"));
	private final JMenuItem copyALLMenuItem = new JMenuItem(resource.getString("main.InstantMessage.menu.AllCopy"));
	private final JMenuItem storeAllIM = new JMenuItem(resource.getString("main.InstantMessage.menu.store"));
	private final JMenuItem storeAllIMXHTML = new JMenuItem(resource.getString("main.InstantMessage.menu.storeToXML"));
	...
	public InstantMessagesPanel(StyleSheet style, ClientSetting setting) {
		...
		copyALLMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE, InputEvent.CTRL_DOWN_MASK));
		...
		for(int i=0; i < InstantMessage.color_set.length;i++)
			style.addRule(".c-"+i+" { color: " + InstantMessage.color_set[i] + ";}");
		...
		instantMessageProvideManager.setAutoFileOutputFlag(setting.getImSetting().getIMAutoLogging());
	}

	/**
	 * インスタントメッセージを追加する
	 * @param im 追加するインスタントメッセージ
	 */
	...
	/**
	 * IM自動保存用出力ストリームを閉じ、フラッシュします。
	 * このメソッドは終了時に一度だけ呼び出してください。
	 */
	public void close() {
	 ...
	 * <p>EditorPaneの文字列を再設定します。</p>
	 * <h3>スレッドセーフ</h3>
	 * <p>テキストの再設定はSwingのイベントディスパッチスレッドで行われます。</p>
	 * <p>このメソッドはイベントディスパッチスレッド以外でも呼び出すことができます。</p>
	 */
	private void resetEditorPaneText() {
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				HTMLInstantMessageFormatter htmlFormatter = new HTMLInstantMessageFormatter(HTMLCS.HTML32.HTMLHeader1, HTMLCS.HTML32.HTMLHeader2, HTMLCS.HTML32.HTMLFooter);
				InstantMessage[] IMArray = instantMessageProvideManager.getInnerIMtoArray();
				...
				editorPane.setText(htmlFormatter.format(IMqueue.toArray(new InstantMessage[0])));
			}
		});
	}
	...
	private void writeOutPlainText(File f, String title){
		try {
			...
			InstantMessage[] buf = instantMessageProvideManager.getInnerIMtoArray();
	...
	private void writeOutXHTML10(File f, String title) {
		try {
			...
			bw.write(HTMLCS.XHTML10S.HTMLHeader1 + title + HTMLCS.XHTML10S.HTMLHeader2);

			HTMLInstantMessageFormatter xhtmlFormatter = new HTMLInstantMessageFormatter(HTMLCS.XHTML10S.HTMLHeader1, HTMLCS.XHTML10S.HTMLHeader2, HTMLCS.XHTML10S.HTMLFooter);
		...
		public void run() {
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					FileNameExtensionFilter filter = new FileNameExtensionFilter(resource.getString("main.InstantMessage.chooser.store"), "txt");
					filechooser.addChoosableFileFilter(filter);
					int selected = filechooser.showSaveDialog(null);
					if (selected == JFileChooser.APPROVE_OPTION){
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss z");
						File file = filechooser.getSelectedFile();
						writeOutPlainText(file, "# Lobby System " + sdf.format(new Date()));
					}
				}
			});
		}
		...
		public void run() {
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					// XHTMLで保存ボタンがクリックされた時
					JFileChooser filechooser = new JFileChooser();
					FileNameExtensionFilter filter = new FileNameExtensionFilter(resource.getString("main.InstantMessage.chooser.storeToXML"), "html", "htm");
					filechooser.addChoosableFileFilter(filter);
					int selected = filechooser.showSaveDialog(null);
					if (selected == JFileChooser.APPROVE_OPTION){
						File file = filechooser.getSelectedFile();
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss z");
						writeOutXHTML10(file, "Lobby IM Log ("+sdf.format(new Date(System.currentTimeMillis()))+")");
					}
				}
			});
		}
	...
	public void registerListeners() {
		...
		storeAllIM.addActionListener(new RunnableCallActionListener(new StoreAllIMRunnable()));
		storeAllIMXHTML.addActionListener(new RunnableCallActionListener(new StoreAllIMXMLRunnable()));
	}
	 ...
	 * <h2>InstantMessageが表示されるTextPaneのキャレット移動ポリシーを変更する</h2>
	 * <p>InstantMessageが表示されるTextPaneのキャレット移動ポリシーを変更します。</p>
	 * <h3>スレッドセーフ</h3>
	 ...
	 * <p>内部のCheckBoxが選択状態を取得する</p>
	 *
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはAbstractButton.isSelectedを呼び出します。</p>
	 * <p>Swingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
	 *
	 * @return trueの場合チェック
	 ...
	 * <p>内部のComboBoxで選択されているインデックスを取得します</p>
	 *
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはJComboBox.getSelectedIndexを呼び出します。</p>
	 * <p>Swingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
	 *
	 * @return 選択されている項目
	 */
	...
	class InnerBottomPanel extends JPanel {
		...
		public InnerBottomPanel() {
			...
			autoScroll = new JCheckBox(resource.getString("main.InstantMessage.autoscroll"));
		 ...
		 * <p>内部のCheckBoxが選択状態を設定する</p>
		 * <h3>スレッドセーフ</h3>
		 * <p>このメソッドはAbstractButton.setSelectedを呼び出します。</p>
		 * <p>Swingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
		 *
		 * @param b trueの場合チェック
		 ...
		 * <p>インデックス anIndex にある項目を選択します。 </p>
		 * <h3>スレッドセーフ</h3>
		 * <p>このメソッドはJComboBox.setSelectedIndexを呼び出します。</p>
		 * <p>Swingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
		 *
		 * @param anIndex 選択するIndex, 0から始まる
		 */
