...
public class InstantMessageProvideManager {
	...
	private final AtomicInteger log_hold_limit = new AtomicInteger(Integer.MAX_VALUE);
	...
	/**
	 * <h1>autoWriteAppend</h1>
	 * このメソッドはフィールドautoFileWriterにより同期化されます。
	 * @param im
	 */
	...
	/**
	 * <h1>getInnerIMtoArray</h1>
	 * <h2>内部で保持しているIMを配列として返す。</h2>
	 * <p>このオブジェクトが保持しているIMを配列として返します。
	 * appendメソッドにより追加した順番は保障されます。</p>
	 * <h3>スレッドセーフ</h3>
	 * <p>このメソッドはフィールドbufにより同期化されます。</p>
	 *
	...
	/**
	 * <h1>toString</h1>
	 * <h2>オブジェクトの文字列表現を返す。</h2>
	 * <p>{@link InstantMessageProvideManager}の toString メソッドは
	 * appendで追加された文字列を適切なテキスト形式でフォーマットした文字列を返します。</p>
	 * <p>このメソッドが返す文字列は内部キューを
	...
	/**
	 * <h1>setAutoFileOutputFlag</h1>
	 * <h2>自動ファイル書き出し機能の有効/無効を切り替える</h2>
	 * <p>オート書き出し機能の有効向こうを切り替えます。
	 * trueが設定された場合、yyyy-mm-dd.txtのファイルストリームを開き、以降のappendメソッド呼び出し時に自動的に書き込まれます。
	 * trueからfalseに切り替えられたとき、既に開かれているストリームは閉じられます。</p>
	 * <p>開かれているストリームはこのメソッドを用いて明示的に閉じられなければなりません。</p>
	 *
	 * @param f trueで有効、falseで無効
	 */
	public void setAutoFileOutputFlag(boolean f){
		...
		if(f){
			...
			try {
				...
				autoFileWriter = new BufferedWriter(new FileWriter(new File(filename),true));
			} catch (IOException e) {
				e.printStackTrace();
				f = false;
			}
	...
	/**
	 * <h1>closeAutoFileOutputStream</h1>
	 * <h2>自動ファイル書き出しで利用されているファイル書き出しストリームを閉じる</h2>
	 * <p>ファイル書き出し用ストリームを閉じます。このメソッドは、setAutoFileOutputFlag(false)と等価です。</p>
	 * <p>setAutoFileOutputFlagまたはこのメソッドは終了時までに明示的に呼び出し、ストリームを閉じる必要があります。</p>
	 *
	 ...
	 * <h1>clearAll</h1>
	 * <h2>内部キューを空にする</h2>
	 * <p>{@link Collection#clear()}を呼び出し、このオブジェクトが内部で保持しているキューを空にします。</p>
	 */
