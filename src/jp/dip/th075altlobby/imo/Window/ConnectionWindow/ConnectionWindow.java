...
public class ConnectionWindow extends JFrame {
	...
	/**
	 * <h1>ConnectionWindow</h1>
	 * <h2>接続ウィンドウのフレームを生成する</h2>
	 * <p>ボタンが押されたときに走るRunnableを指定して、インスタンスを生成します。</p>
	 * <p>指定されるRunnableは別スレッドで実行されます。Swingのイベントディスパッチスレッドからは呼ばれません。</p>
	 *
	 */
	public ConnectionWindow(String[] server_list, Runnable connectionButtonRunnable, Runnable settingButtonRunnable) {
	 ...
	 * <p>内部で保持しているパネルのコンボボックスで選択された文字列を取得します。</p>
	 * <p>このメソッドが呼び出すConnectionWindowInnerPanel1.getSelectedIndexは
	 * 内部でJComboBox.getSelectedIndexを呼び出すためSwingのイベントディスパッチスレッド以外から呼び出すことはできません。</p>
	 *
	 * @return 選択された文字列
	 */
