...
public class ClientThread implements Runnable {
    ...
    private final AtomicInteger myUserState = new AtomicInteger(
            UserInfo.WAITING);
    ...
    public ClientThread(MainWindow mainWindow, Socket s, ClientSetting setting)
            throws IOException {
        _socket = s;
     ...
     * 
     * この状態は特別な状態です。 ホスト接続待ちユーザに対する接続要求時以外に呼び出すべきではありません。
     ...
     * <p>
     * このメソッドは捕捉しているcasterプロセスへの書き出し機能を提供します。casterを捕捉していない場合、このメソッドは何も実行しません。
     * </p>
     * <p>
     * このメソッドは柔軟な操作を可能としますが注意して利用する必要があります。詳細は
     * {@link IOStreamCasterAdapterWithOutputCallback#writeCasterStream(String)}
     * を参照してください。
     * </p>
     * 
     * @param s
    ...
    public void run() {
        ...
        try {
            InputThread in = new InputThread(_socket, _socket.getInputStream(),
                    out);
            ...
            scheduler.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    _ioThread.put(new WrappedData(WrappedData.KEEP_ALIVE,
                            new byte[0]));
                }
            }, ClientConstants.KEEP_ALIVE_TIME_FIRST_DELAY,
                    ClientConstants.KEEP_ALIVE_TIME_DELAY, TimeUnit.SECONDS);
    ...
    public void connectCaster(UserInfo info) throws IOException {
        ...
        // caster = new IOStreamCasterAdapter(new CasterEventListener(), new
        // CasterStateChangeListener(),
        // setting.getCasterSetting().getCasterPath(), "-i" + ipString + " -p" +
        // port);
        caster = new IOStreamCasterAdapterWithOutputCallback(
                ...
                _mainWindow.getOutputCasterMessageCallback(), _setting
                        .getCasterSetting().getCasterPath(), "-i" + ipString
                        + " -p" + port);
    }

    /**
     * ClientThreadを安全な形で終了します。 このメソッドの呼び出しによって、socketがクローズされることはありません。
    ...
    class InputThread implements Runnable {
        ...
        public void run() {
            ...
            try {
                try {
                    ...
                    _out.put(new WrappedData(WrappedData.EXIT_DUMMY,
                            new byte[0]));
                } catch (IOException e) {
                ...
                logger.log(Level.SEVERE,
                        "RuntimeExceptionをcatchしました。処理を続行できません。以下にトレースを示します。",
                        e);
            }
        ...
        private void consume(WrappedData data) throws IOException {
            ...
            case WrappedData.RESPONSE_KEEP_ALIVE:
                ...
                break;
            default:
                logger.warning("サーバーから未知のコマンドが届きました。command = " + command
                        + "はサポートされていません。");
            }
        }
        ...
        /**
         * <h1>responseModifyState</h1> <h2>ModifyStateの結果を処理する</h2>
         * <p>
         * サーバーから送信された状態変更の結果を処理します。
         * </p>
         * <p>
         * このメソッドは{@link MessageInputPanel#switchButton(int)}を呼び出しボタンの状態を変更し、
         * {@link MessageInputPanel#setUserStateTextLabelState(byte)}
         * を呼び出しテキストの変更を行った後、 {@link MainWindow#myUserState}の値をアトミックに変更します。
         * </p>
         * 
         * @param rawData
         *            サーバーから送られたバイト列
        ...
        /**
         * <h1>distributedClientToClientCommand</h1> <h2>
         * サーバーから配信されたクライアント間コマンドの処理をする</h2>
         * <p>
         * サーバーから送られたクライアント間コマンドを処理します。
         * </p>
         * 
         * @param rawData
         *            サーバーから送られたバイト列
         ...
         *             入出力例外が発生した場合
         */
        private void distributedClientToClientCommand(byte[] rawData)
                throws IOException {
            ClientToClientCommand ccc = DataSplitter
                    .toDistributedClientToClientCommand(rawData);
            ...
            case ClientToClientCommand.ACCEPTED_POST_FIGHT:
                ...
                try {
                    ...
                    logger.log(
                            Level.WARNING,
                            "connectCasterでIO例外をcatchしました。Casterを起動できません。以下にトレースを示します。",
                            e);
                }
                break;
            case ClientToClientCommand.POST_FIGHT:
                ...
                case UserInfo.HOST_GATHERING_WAIT:
                    try {
                        closeCaster();
                        String IPString = IPConverter.toString(info
                                .getIpAddress());
                        // caster = new IOStreamCasterAdapter(new
                        // CasterEventListener(), new
                        // CasterStateChangeListener(),
                        // setting.getCasterSetting().getCasterPath(), "-w");
                        caster = new IOStreamCasterAdapterWithOutputCallback(
                                _mainWindow.getCasterEventListener(),
                                _mainWindow.getCasterStateChangeListener(),
                                _mainWindow.getOutputCasterMessageCallback(),
                                _setting.getCasterSetting().getCasterPath(),
                                "-i" + IPString, "-w");
                        ...
                        _ioThread
                                .put(new WrappedData(
                                        ...
                                        DataWrapper.wrapPostClientToClientCommand(
                                                ccc.getFromUID(),
                                                ClientToClientCommand.ACCEPTED_POST_FIGHT)));
                        ...
                        _mainWindow.putSystemMessage(
                                ...
                                info.getPublicName()
                                        ...
                                        + messageMap
                                                .get("main.message.connected"));
                    } catch (IOException e) {
                        JOptionPane.showMessageDialog(null,
                                "Caster起動時にエラーが発生しました。ログにトレースを出力します。",
                                "IOException", JOptionPane.ERROR_MESSAGE);
                        logger.log(Level.WARNING,
                                "接続要求を受けましたがCasterの起動に失敗しました。"
                        ...
                        _ioThread
                                .put(new WrappedData(
                                        ...
                                        DataWrapper.wrapPostClientToClientCommand(
                                                ccc.getFromUID(),
                                                ClientToClientCommand.PROCESS_START_ERROR)));
                    }
                    break;
                case UserInfo.HOST_GATHERING:
                    ...
                    _mainWindow.putSystemMessage("System",
                            info.getPublicName() + "@"
                                    + info.getUID().substring(0, 3)
        ...
        /**
         * <h1>storedIM</h1> <h2>サーバーに保存されたIMを処理する</h2>
         * <p>
         * サーバーに保存されているIMを処理します。
         * </p>
         * <p>
         * サーバーは要求を拒否することがあります。
         * </p>
        ...
        private void storedIM(byte[] rawData) throws IOException {
            ResponseStoredInstantMessages im = DataSplitter
                    .toResponseStoredInstantMessages(rawData);
        ...
        /**
         * <h1>postedIM</h1> <h2>ポストされたIMを処理する</h2>
         * <p>
         * サーバーから送られたIM情報を処理します。
         * </p>
         * 
         * @param rawData
         *            サーバーから送られたバイト列
        ...
        private void postedIM(byte[] rawData) throws IOException {
            ...
            // 対戦中に音を鳴らさない + 対戦中
            if (_setting.getImsoundSetting().getNonIMSoundIfPlaying()
                    && (myUserState.get() == UserInfo.FIGHTING))
                return;

            // フォーカスがあるときに音を鳴らさない + フォーカスがある
            if (_setting.getImsoundSetting().getNonIMPlayingIfWindowFocused()
            ...
            if (im.getTo().equals("all")) {
                ...
                if (_setting.getImsoundSetting().getWholeIMPlaySound()) {
                    try {
                        ClipPlayer player = new ClipPlayer(_setting
                                .getImsoundSetting().getWavePath());
                        player.play();
                ...
                try {
                    ClipPlayer player = new ClipPlayer(_setting
                            .getImsoundSetting().getToMeWavePath());
                    player.play();
                } catch (LineUnavailableException e) {
                    logger.warning("LineUnavailableException");
                } catch (UnsupportedAudioFileException e) {
        ...
        private void responceUserlist(byte[] rawData) throws IOException {
             ...
             * 
             * @Override public void run() { for(int i=0;i < usersArray.length;
             * i++) { userinfoTablePanel.addUserInfo(usersArray[i]); } } });
             */
        }
        ...
        /**
         * バージョンのチェック要求を処理する。 このサーバーが対応しているプロトコルのバージョンとサーバー自身のバージョンを送信します。
         * 
         * @param rawData
         *            サーバーから送られたバイト列
         * @throws IOException
        ...
        private void checkVersion(byte[] b) throws IOException {
            ...
            logger.log(Level.INFO,
                    "PROTOCOL_VERSION = " + ver.getProtocolVersion());
    ...
    class OutputThread extends AbstractOutputThread {
         ...
         * このメソッドがブロックされることはありません。 キューに追加されたデータは、送信可能であればすぐサーバーに送信されます。
         * 
         * @param data
         *            キューに追加されるデータ
         */
